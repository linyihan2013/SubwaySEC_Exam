...
/**
 * <p>
 * Title: 控制中心，负责将请求命令分发到向对象。 功能已实现，考生无需关注
 * </p>
 *
 * <p>
 * Description: 各方法按要求返回，程序库会组装报文输出
 * </p>
 *
 * <p>
...
public class SubwaySECControlCenter {
	 ...
	 *
	 * @param command
	 *            入参 command - 执行命令 enterSite - 进站名 exitSite - 出站 名 cardType -
	 *            卡类型 money - 余额 cardId - 卡号 enterTime - 进站时间 exitTime - 出站时间
	 * @return Result 响应消息
	 */
	public OperationResult execute(Command command) {
		...
		try {
			...
			case CMD_ONCE: {
				/**
				 * 单程票购买 responseBody必须包含操作结果、卡号和余额
				 */
			...
			case CMD_BUY: {
				/**
				 * 办理乘车卡 responseBody必须包含操作结果、卡号、卡类型和余额
				 */
			...
			case CMD_CONSUME: {
				/**
				 * 乘车扣费 responseBody必须包含操作结果、卡号、卡类型和余额
				 */
				Card card = passenger.takeSubway(command.getCardId(), command.getEnterStation(), command.getEnterTime(),
						command.getExitStation(), command.getExitTime());
			...
			case CMD_RECHARGE: {
				/**
				 * 充值 responseBody必须包含操作结果、卡号、卡类型和余额
				 */
			...
			case CMD_DELETE: {
				/**
				 * 注销地铁卡 responseBody必须包含操作结果、卡号和卡类型
				 */
			...
			case CMD_QUERY_RECORD: {
				/**
				 * 查询历史消费记录 responseBody必须包含操作结果、卡号、卡类型和消费记录
				 */
